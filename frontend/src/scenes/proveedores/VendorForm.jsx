...
const VendorForm = ({ open, onClose, onCreated }) => {
  ...
  const submit = async () => {
    try {
      await Vendors.createVendor(form);
      setSnack({ open: true, msg: "Proveedor creado", sev: "success" });
      onCreated?.();
    } catch (e) {
      setSnack({ open: true, msg: "Error al crear", sev: "error" });
    }
  };
