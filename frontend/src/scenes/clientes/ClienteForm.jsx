...
const ClienteForm = ({ open, onClose, onCreate }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const created = await Customers.createCustomer(payload);
      onCreate?.(created);
      setSnackbarMessage('Cliente creado');
        ...
        <DialogContent sx={{ backgroundColor: colors.primary[400] }}>
          <Box p={3} borderRadius={2}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}><TextField name="tax_condition" label="Condición IVA" fullWidth margin="normal" value={formData.tax_condition} onChange={handleChange} placeholder="RI / CF / Exento / Monotributo" /></Grid>
